    ...
    ui(new Ui::MainWindow)
{
    ...
    QObject::connect(ui->action_savesql, &QAction::triggered, this, &MainWindow::askSavesql);
    ...
    QObject::connect(this, &MainWindow::sendAskSavesql, ui->myGraphicsView, &MyGraphicsView::replySavesql);
...
{
    ui->action_savesql->setDisabled(!connected);
    if (!connected)
    {
        QMessageBox message(this);
        message.setWindowTitle("哎呀!");
        message.setIconPixmap(QPixmap(":/icon/img/warn.png"));
        message.setText("数据库连接失败，你将无法使用保存至数据库功能");
        message.exec();
    }
}
...
void MainWindow::askSavesql()
{
    emit sendAskSavesql();
    QMessageBox message(this);
    message.setWindowTitle("信息");
    message.setIconPixmap(QPixmap(":/icon/img/tips.png"));
    message.setText("地图信息已更新至数据库");
    message.exec();
}
