...
void AbstractGraph::savesql()
{
    /* 数据库操作 */
    QString ts;
    QSqlQuery query(DB);
    ts = "DELETE FROM Station;";
    query.exec(ts);
    ts = "DELETE FROM Line;";
    query.exec(ts);
    ts = "DELETE FROM Edge;";
    query.exec(ts);
    ts = "DELETE FROM Belong;";
    query.exec(ts);

    //点
    for (vNode v : vInfo) {
        if (v.isExist){
            ts = "INSERT INTO Station VALUES (";

            ts += QString::number(v.sationAt->id);
            ts += ",'";
            ts += v.sationAt->name;
            ts += "',";
            ts += QString::number(v.sationAt->size);
            ts += ",";
            QPointF center = v.sationAt->getCenter();
            ts += QString::number(center.x(), 'f', 2);
            ts += ",";
            ts += QString::number(center.y(), 'f', 2);
    ...
    for (int i = 0; i < lInfo.size(); ++i) {
        ...
        if (l.isExist){
            ...
            ts += "','";
            QColor c = l.color;
            ts += QString::number(qRgb(c.red(),c.green(),c.blue()), 16);
            ts += "',";
            ts += QString::number(l.isCircle);
    ...
    for (eNode e : eInfo) {
        if (e.isExist){
            // 边
            ts = "INSERT INTO Edge VALUES (";

            ts += QString::number(e.edgeAt->id);
            ts += ",";
            ts += QString::number(e.from);
            ts += ",";
            ts += QString::number(e.to);
            ts += ",";
            ts += QString::number(e.weight, 'f', 2);
            ts += ",";
            ts += QString::number((int)e.edgeAt->style);
            ...
            for (int i = 0; i < 2; ++i) {
                if (e.belongLine[i]) {
                    ts = "INSERT INTO Belong VALUES (";

                    ts += QString::number(e.edgeAt->id);
                    ts += ",";
                    ts += QString::number(e.belongLine[i]);
...
void MyGraphicsView::replySavesql()
{
    tupo.savesql();
}
